...
func InitComponents(app *widgets.QMainWindow) {
	...
	btn1.ConnectClicked(buttonClicked(app, btn1))
	btn2.ConnectClicked(buttonClicked(app, btn2))
}

func buttonClicked(app *widgets.QMainWindow, btn *widgets.QPushButton) func(checked bool) {
	return func(checked bool) {
		fmt.Printf("======== %#v\n", btn.ObjectName())
		fmt.Printf("======== %#v\n", app.Sender().ObjectName())
		if btn.Text() == "Button2" {
			app.StatusBar().ShowMessage(btn.Text(), -1)
		}
	}
}
